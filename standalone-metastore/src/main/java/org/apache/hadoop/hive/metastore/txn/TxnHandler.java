...
abstract class TxnHandler implements TxnStore, TxnStore.MutexAPI {
  ...
  private List<Long> openTxns(Connection dbConn, Statement stmt, OpenTxnRequest rqst)
    ...
    try {
      ...
      for (long i = first; i < first + numTxns; i++) {
        txnIds.add(i);
        rows.add(i + "," + TxnStatus.OPEN + "," + now + "," + now + ",?,?," + txnType.getValue());
    ...
    try {
      ...
      try {
        ...
        StringBuilder prefix = new StringBuilder("select TXN_ID, TXN_TYPE from TXNS where TXN_STATE = ")
          .append(TxnStatus.OPEN).append(" and TXN_TYPE != ").append(TxnType.READ_ONLY.getValue())
          .append(" and ");
        ...
        if (rs != null && rs.next()) {
          ...
          rs = stmt.executeQuery
            (sqlGenerator.addLimitClause(1, "committed.ws_txnid, committed.ws_commit_id, committed.ws_database," +
              ...
              " and (committed.ws_operation_type=" + OperationType.UPDATE +
              " OR cur.ws_operation_type=" + OperationType.UPDATE + ")"));
          if (rs.next()) {
            //found a conflict
    ...
    try {
      ...
      try {
        ...
        if (!isTxnsOpenAndNotReadOnly(txnIds, stmt)) {
          String errorMsg = "Write ID allocation on " + Warehouse.getQualifiedName(dbName, tblName)
              + " failed for input txns: "
              + getAbortedAndReadOnlyTxns(txnIds, stmt)
              + getCommittedTxns(txnIds, stmt);
          LOG.error(errorMsg);

          throw new IllegalStateException("Write ID allocation failed on " + Warehouse.getQualifiedName(dbName, tblName)
  ...
  public LockResponse lockMaterializationRebuild(String dbName, String tableName, long txnId)
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("Acquiring lock for materialization rebuild with txnId={} for {}",
          JavaUtils.txnIdToString(txnId), Warehouse.getQualifiedName(dbName, tableName));
    }
   ...
   * @throws MetaException
   */
  private TxnRecord lockTransactionRecord(Statement stmt, long txnId, TxnStatus txnState) throws SQLException, MetaException {
