...
EXPLAIN SELECT sum(cfloat) FROM druid_table_n0 WHERE cstring1 != 'en' group by 1.011;
SELECT sum(cfloat) FROM druid_table_n0 WHERE cstring1 != 'en' group by 1.011;

-- gby with multiple constant keys
EXPLAIN SELECT sum(cfloat) FROM druid_table_n0 WHERE cstring1 != 'en' group by 1.011, 3.40;
SELECT sum(cfloat) FROM druid_table_n0 WHERE cstring1 != 'en' group by 1.011, 3.40;

-- group by with constant folded key
EXPLAIN SELECT sum(cint) FROM druid_table_n0 WHERE cfloat= 0.011 group by cfloat;
SELECT sum(cint) FROM druid_table_n0 WHERE cfloat= 0.011 group by cfloat;

-- group by key is referred in select
EXPLAIN SELECT cfloat, sum(cint) FROM druid_table_n0 WHERE cfloat= 0.011 group by cfloat;
SELECT cfloat, sum(cint) FROM druid_table_n0 WHERE cfloat= 0.011 group by cfloat;
