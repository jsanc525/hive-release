...
public class LoadPartitions {
  ...
  private void addConsolidatedPartitionDesc() throws Exception {
    ...
    int maxTasks = context.hiveConf.getIntVar(REPL_LOAD_PARTITIONS_BATCH_SIZE);
    ...
    while (currentPartitionCount < totalPartitionCount) {
      ...
      for (AddPartitionDesc addPartitionDesc : partitionBatch) {
        ...
        AddPartitionDesc.OnePartitionDesc newPartition = new AddPartitionDesc.OnePartitionDesc(
          src.getPartSpec(), replicaWarehousePartitionLocation.toString(), partParams);
        newPartition.setInputFormat(src.getInputFormat());
        newPartition.setOutputFormat(src.getOutputFormat());
        newPartition.setNumBuckets(src.getNumBuckets());
        newPartition.setCols(src.getCols());
        newPartition.setSerializationLib(src.getSerializationLib());
        newPartition.setSerdeParams(src.getSerdeParams());
        newPartition.setBucketCols(src.getBucketCols());
        newPartition.setSortCols(src.getSortCols());
        newPartition.setColStats(src.getColStats());
        newPartition.setWriteId(src.getWriteId());
