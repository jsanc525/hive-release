...
import static org.apache.hadoop.hive.conf.HiveConf.ConfVars.REPL_LOAD_PARTITIONS_BATCH_SIZE;
...
public class LoadPartitions {
  ...
  private final AddPartitionDesc.OnePartitionDesc lastReplicatedPartitionDesc;
  ...
  public LoadPartitions(Context context, ReplLogger replLogger, TaskTracker tableTracker,
    ...
    this(context, replLogger, tableContext, tableTracker, event, dbNameToLoadIn, null,
      null, PartitionState.Stage.PARTITION);
  }
  ...
  public LoadPartitions(Context context, ReplLogger replLogger, TableContext tableContext,
                        ...
                        AddPartitionDesc.OnePartitionDesc lastReplicatedPartitionDesc,
  ...
  private boolean isMetaDataOp() {
    return HiveConf.getBoolVar(context.hiveConf, REPL_DUMP_METADATA_ONLY);
  }
   ...
   */
  private void addConsolidatedPartitionDesc(AddPartitionDesc lastAlterTableAddPartitionDesc) throws Exception {
    int maxTasks = 0;
    ...
    if (lastAlterTableAddPartitionDesc != null) {
      while (partitionIterator.hasNext()) {
        ...
        if (lastAlterTableAddPartitionDesc.getPartition(0).getPartSpec()
          .equals(addPartitionDesc.getPartition(0).getPartSpec())) {
          break;
        }
      }
    }
    ...
    while (currentPartitionCount < totalPartitionCount) {
      List<AddPartitionDesc.OnePartitionDesc> partitions = new LinkedList<>();
      ...
      List<AddPartitionDesc> partitionBatch = partitionDescs.subList(currentPartitionCount,
        toPartitionCount);
      for (AddPartitionDesc addPartitionDesc : partitionBatch) {
        AddPartitionDesc.OnePartitionDesc src = addPartitionDesc.getPartition(0);
        ...
        partitions.add(new AddPartitionDesc.OnePartitionDesc(
      ...
      AddPartitionDesc consolidatedPartitionDesc = new AddPartitionDesc(tableDesc.getDatabaseName(),
  ...
  private void addPartition(boolean hasMorePartitions, AddPartitionDesc addPartitionDesc)
          throws Exception {
   ...
   */
  private boolean addTasksForPartition(Table table, AddPartitionDesc addPartitionDesc,
                                    AddPartitionDesc.OnePartitionDesc lastPartSpec)
    ...
    if (isMetaDataOp() || (!event.replicationSpec().isMigratingToExternalTable() && TableType.EXTERNAL_TABLE.equals(table.getTableType()))) {
      tracker.addTask(addPartTask);
      return true;
    }
    //Add Copy task for all partitions
    boolean lastProcessedStageFound = false;
    for (AddPartitionDesc.OnePartitionDesc partSpec : addPartitionDesc.getPartitions()) {
      if (!tracker.canAddMoreTasks()) {
